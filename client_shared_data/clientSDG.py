...
class Client:
    ...
    def start(self):
        ...
        print(f"[CLIENT {self.client_id}] Main execution runtime processing initialized...")
        
        try:
            while True:
                if self.active_round and self.msg_payload is not None:
                    # Execute all heavy math processing sequentially on the safe main thread
                    self.load_state(self.msg_payload)
                    
                    print(f"[CLIENT {self.client_id}] Starting local training round...")
                    self.train(epochs=1)
                    
                    print(f"[CLIENT {self.client_id}] Commencing local threshold sweep calibration...")
                    metrics = self.evaluate()

                    # Send unified payload update back to server
                    ...
                    print(f"[CLIENT {self.client_id}] Metrics packed and dispatched back to server.\n")
                    
                    # Reset synchronization states for the round
                    self.active_round = False
                    self.msg_payload = None
                
                time.sleep(0.1)  # Keeps CPU from running at 100% while idling
        except KeyboardInterrupt:
            print(f"\n[CLIENT {self.client_id}] Termination command captured. Stopping runtime...")
            self.mqtt.loop_stop()
